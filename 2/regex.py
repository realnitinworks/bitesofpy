...
def extract_course_times(course=COURSE):
    """Return the course timings from the passed in
       ...
       Return this list.
    """
    times_regex = re.compile(r'\d\d:\d\d')
    return times_regex.findall(course)
...
def get_all_hashtags_and_links(tweet=TWEET):
    """Get all hashtags and links from the tweet text
       ...
       Return this list.
    """
    hashtags_links_regex = re.compile(r'http:.*?\s|#\w+')
    return [
        string.strip()
        for string in hashtags_links_regex.findall(tweet)
    ]
...
def match_first_paragraph(html=HTML):
    """Extract the first paragraph of the passed in
       ...
       Return this string.
    """
    first_para_regex = re.compile(r'<p>(.*?)</p>')
    mo = first_para_regex.search(html)
    return mo.group(1)
