...
def check_bt(donor, recipient):
    ...
    """
    allowed_types = {str, int, Bloodtype}

    if not any(isinstance(donor, allowed) for allowed in allowed_types):
        raise TypeError

    if not any(isinstance(recipient, allowed) for allowed in allowed_types):
        raise TypeError
