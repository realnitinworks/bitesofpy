...
def operas_both_at_premiere(guest, composer):
    ...
    input_composers = [guest, composer]
    if any(composer not in composers for composer in input_composers):
        raise ValueError("Not a valid composer")

    death_dates = {
        _get_date(composers[person].died)
        for person in input_composers
    }

    born_dates = {
        _get_date(composers[person].born)
        for person in input_composers
    }

    operas_filtered_by_author = (
        opera
        for opera in operas
    ...
    return (
        opera.play
        for opera in operas_filtered_by_author
        if all(
            _get_date(opera.date) < dt
            for dt in death_dates
        )
        and
        all(
            _get_date(opera.date) > dt
            for dt in born_dates
        )
    )
