...
def parse_social_platforms_string():
    ...
    social_platform_regex = re.compile(r'(Twitter|Facebook|Reddit)\s+Min:\s(\d+)\s+Max:\s(\d+)\s+Can contain:\s(.*)')
    return {
        platform: Validator(range=range(int(range_min), int(range_max)), regex=re.compile(f'^[{regex}]+$'))
        for platform, range_min, range_max, regex in social_platform_regex.findall(social_platforms)
    }
...
def validate_username(platform, username):
    ...
    return validator.regex.search(username) is not None
