...
def get_ordinal_suffix(number):
    ...
    # Exception to the rule: all the teen numbers ending with 11, 12, 13
    try:
        return ordinal_mapping[str(number)[-2:]].format(number)
    except KeyError:
        pass

    # All the other numbers
    try:
        return ordinal_mapping[str(number)[-1]].format(number)
    except KeyError:
        return f"{number}th"
