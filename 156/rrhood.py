...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    characters_lower_cased = [
        tuple(c.lower() for c in character) if isinstance(character, tuple)
    ...
    for number, line in enumerate(lines):
        for character in characters_lower_cased:
            if isinstance(character, tuple):
                for synonym in character:
                    if synonym in line:
                        character_index[character[0]].append(number)
                        break
            else:
                if character in line:
                    character_index[character].append(number)
