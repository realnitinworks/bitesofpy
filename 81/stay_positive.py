...
def filter_tweets_on_polarity(tweets, keep_positive=True):
    ...
    postive_tweets = [tweet for tweet in tweets if tweet.polarity > 0]
    negative_tweets = [tweet for tweet in tweets if tweet.polarity < 0]
    return postive_tweets if keep_positive else negative_tweets
...
def order_tweets_by_polarity(tweets, positive_highest=True):
    ...
    return sorted(
        tweets,
        key=lambda tweet: tweet.polarity,
        reverse=positive_highest
    )
