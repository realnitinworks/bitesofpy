...
class BridgeHand:
    def __init__(self, cards: Sequence[Card]):
        ...
        if not all(isinstance(card, Card) for card in cards):
            raise ValueError("Not a sequence of <Card>s")

        if len(cards) != 13:
            raise ValueError("Not a sequence of length 13")
    ...
    def __str__(self) -> str:
        ...
        for suit, ranks in self.sorted_cards_dict.items():
            ret_str += f"{suit}:{ranks} "

        return ret_str.strip()

    @property
    def sorted_cards_dict(self):
        sorted_cards = sorted(
            sorted(self.cards, key=lambda card: card.rank.value),
            key=lambda card: card.suit.name,
            reverse=True
        )
        cards_dict = defaultdict(str)
        for card in sorted_cards:
            cards_dict[card.suit.name] += card.rank.name
    ...
    def doubletons(self) -> int:
        ...
        return sum(
            len(ranks) == 2
            for ranks in self.sorted_cards_dict.values()
        )
    ...
    def voids(self) -> int:
        ...
        """
        return sum(
            suit.name not in self.sorted_cards_dict
            for suit in Suit
        )
    ...
    def ssp(self) -> int:
        """ Return the number of short suit points in this hand.
            ...
            voids 3 points
        """
        short_suits = [self.doubletons, self.singletons, self.voids]
        return sum(
            suit * point
            for suit, point in zip(short_suits, SSP.values())
        )
