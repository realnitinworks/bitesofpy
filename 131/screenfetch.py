...
def sysinfo_scrape(output):
    ...
    sysinfo_text = output.splitlines()
    sysinfo_text = [line.split("  ")[-1].strip() for line in sysinfo_text]
    sysinfo_text = [line for line in sysinfo_text if line]

    sysinfo_dict = {}
    sysinfo_dict['Name'] = sysinfo_text[0]
    sysinfo_dict.update(dict(line.split(":") for line in sysinfo_text[1:]))
    sysinfo_dict = {k: v.strip() for k, v in sysinfo_dict.items()}
    return sysinfo_dict
