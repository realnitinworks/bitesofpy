...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    events = (
        json.loads(event.contents[0])
        for event in soup.select('script[type="application/ld+json"]')
    )
    events = (event for event in events if "PyCon" in event['name'])
    ...
    for event in events:
        ...
        pycon_events.append(PyCon(name, city, country, start_date, end_date, url))
...
def filter_pycons(pycons: List[PyCon],
    ...
    return [
        ...
        for pycon in pycons
        if (
            get_continent(pycon.country) == continent and
            pycon.start_date.year == year
        )
    ]
